...
const Player = ({fontCol,bgCol}) => {
  ...
  const defaultUrl = 'https://docs.google.com/document/d/1-Bi6NTfIqHHF0ir-Y2Ycng3YQi-vFr5TP-wrfnyVIoc/edit' || 'https://www.loom.com/embed/41f37898926845b797fca215f04ce57c?sid=ea09df54-09cd-454d-8c16-557a11106a12';
  const id = defaultUrl.match(/\/d\/([a-zA-Z0-9-_]+)/)[1];
  const defUrl = `https://www.loom.com/embed/41f37898926845b797fca215f04ce57c?sid=ea09df54-09cd-454d-8c16-557a11106a12`;
  ...
  useEffect(() => {
    if (storedurl) {
      setUrl(storedurl);
      const loomIdMatch = storedurl.match(/\/d\/([a-zA-Z0-9-_]+)/);
      if (loomIdMatch && loomIdMatch[1]) {
        setEmbedUrl(`https://docs.google.com/document/d/${loomIdMatch[1]}/preview`);
        setShowWarning(false);
      } else {
        // setShowWarning(true);
        setEmbedUrl(defUrl);
      ...
      setUrl(defaultUrl);
      const loomIdMatch = defaultUrl.match(/\/d\/([a-zA-Z0-9-_]+)/);
      if (loomIdMatch && loomIdMatch[1]) {
        setEmbedUrl(`https://docs.google.com/document/d/${loomIdMatch[1]}/preview`);
        setShowWarning(false);
      } else {
  ...
  useEffect(() => {
    if (storedisEditing) {
      ...
      if (storedisEditing) {
        const loomIdMatch = url.match(/\/d\/([a-zA-Z0-9-_]+)/);
        if (loomIdMatch && loomIdMatch[1]) {
          setEmbedUrl(`https://docs.google.com/document/d/${loomIdMatch[1]}/edit`);
        }
      }
    }
  }, [storedisEditing]);
  ...
  const handleUrlChange = (event) => {
    ...
    // setUrlSetting(false) ; 
    const loomIdMatch = inputUrl.match(/\/d\/([a-zA-Z0-9-_]+)/);
    if (loomIdMatch && loomIdMatch[1]) {
      setEmbedUrl(`https://docs.google.com/document/d/${loomIdMatch[1]}/preview`);
      // setShow(false);
      setShowWarning(false);
  ...
  const toggleEditMode = () => {
    ...
    if (newEditMode) {
      ...
      if (slideIdMatch && slideIdMatch[1]) {
        setEmbedUrl(`https://docs.google.com/document/d/${slideIdMatch[1]}/edit`);
      }
    } else {
      const slideIdMatch = url.match(/\/d\/(.+?)\//);
      if (slideIdMatch && slideIdMatch[1]) {
        setEmbedUrl(`https://docs.google.com/document/d/${slideIdMatch[1]}/preview`);
      }
    }
  };
  ...
  return (
    ...
    {!cookieConsent && (
      ...
      {!submitted && (<div className="company">
        ...
        <div className="name" >
          <b><span style={{ height:"19px", color:fontCol }}>Google Docs Embed for monday.com</span></b>
